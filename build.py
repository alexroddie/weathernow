...
def fetch_weather():
    lat = 56.60712432274488
    lon = -3.0858753489219812
...
function renderDashboard() {
  ...
  if (current_h_int >= 18) { laterHTML = `<div class="v-time">Sleep well</div>`; } else {
    ...
    let lmp = 0, lc = 99, lwm = 0, lgm = 0; let lphRes = -1, lphCode = 0, lphTemp = 0;
    for (let j = scanStart; j <= endOfDayIdx; j++) {
      let p = d.hourly.precipitation_probability[j]; if (p > lmp) lmp = p;
      if (p >= 40 && lphRes === -1) { lphRes = parseInt(d.hourly.time[j].split("T")[1].split(":")[0]); lphCode = d.hourly.weather_code[j]; lphTemp = d.hourly.temperature_2m[j]; }
      if (d.hourly.weather_code[j] < lc) lc = d.hourly.weather_code[j]; if (d.hourly.wind_speed_10m[j] > lwm) lwm = d.hourly.wind_speed_10m[j]; if (d.hourly.wind_gusts_10m[j] > lgm) lgm = d.hourly.wind_gusts_10m[j];
    }
  ...
  let nLow = 100, nWi = 0, nGu = 0, nMp = 0; let nPhRes = -1, nPhCode = 0, nPhTemp = 0;
  const nightStartIdx = endOfDayIdx - 1; const nightEndIdx = endOfDayIdx + 7;   
  for (let j = nightStartIdx; j <= nightEndIdx; j++) {
    if (j >= hr_now) {
      if (d.hourly.temperature_2m[j] < nLow) nLow = d.hourly.temperature_2m[j]; if (d.hourly.wind_speed_10m[j] > nWi) nWi = d.hourly.wind_speed_10m[j]; if (d.hourly.wind_gusts_10m[j] > nGu) nGu = d.hourly.wind_gusts_10m[j];
      let p = d.hourly.precipitation_probability[j]; if (p > nMp) nMp = p;
      if (p >= 40 && nPhRes === -1) { nPhRes = parseInt(d.hourly.time[j].split("T")[1].split(":")[0]); nPhCode = d.hourly.weather_code[j]; nPhTemp = d.hourly.temperature_2m[j]; }
    }
  }
  ...
  let tMax = -99, tMin = 99, tomWi = 0, tomGu = 0; let tomPhRes = -1, tomPhCode = 0, tomPhTemp = 0; let daylightRainCount = 0, daylightSnowCount = 0;
  ...
  for (let j = tomStartIdx; j <= tomEndIdx; j++) {
    ...
    if (p >= 40 && tomPhRes === -1) { tomPhRes = parseInt(d.hourly.time[j].split("T")[1].split(":")[0]); tomPhCode = d.hourly.weather_code[j]; tomPhTemp = d.hourly.temperature_2m[j]; }
    if (j <= daylightEndIdx && p >= 40) { daylightRainCount++; let hc = d.hourly.weather_code[j]; if (hc >= 71 && hc <= 86) daylightSnowCount++; }
  }
  ...
  if (tomPhRes !== -1) { 
    ...
    tomStr += `Expect ${type} at about ${formatAmPm(tomPhRes)}. `; 
  } else { tomStr += `Likely dry. `; }
  tomStr += tomGu < 10 ? `Probably calm.` : `Winds ${Math.round(tomWi)}/${Math.round(tomGu)}mph.`;

  document.getElementById('dashboard').innerHTML = `
    <div class="top-row">
      <div class="col-left"><div class="loc">Weather now</div><div class="sub-loc">Castleton of Eassie</div><div class="date-line">${dateString}</div><div class="sun">Sunrise ${formatSunTime(d.daily.sunrise[day_now])} <span style="font-size: 10px; margin: 0 1px;">&#9728;&#xFE0E;</span> Sunset ${formatSunTime(d.daily.sunset[day_now])}</div></div>
